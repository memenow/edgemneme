...
describe("GitHub sync policy", () => {
  ...
  it("fails every configured repository before metadata access on baseline expansion", async () => {
    const repositories = [
      repositoryRow("repository-1", 42),
      repositoryRow("repository-2", 84)
    ];
    const failures: unknown[][] = [];
    const database = createScheduledDatabase(
      repositories,
      {
        credential_version: "credential-current",
        user_id: 7,
    ...
    const fetcher = vi.fn<typeof fetch>().mockImplementation(async (input) => {
      ...
      if (url.pathname === "/user/repos") {
        return Response.json([
          ...
          {
            id: 126,
            full_name: "memenow/unapproved",
            owner: { id: 7 },
            permissions: { pull: true, push: false, admin: false }
          }
        ]);
      }
      throw new Error(`unexpected metadata request: ${url.pathname}`);
    });
    ...
    expect(
      fetcher.mock.calls.some(([input]) =>
        new URL((input as Request).url).pathname.startsWith("/repos/")
      )
    ).toBe(false);
  ...
  it("fails every configured repository without network access when baseline is missing", async () => {
    ...
    const failures: unknown[][] = [];
    const database = createScheduledDatabase(repositories, null, failures);
    const fetcher = vi.fn<typeof fetch>();
  ...
  it("selects enabled GitHub repositories case-insensitively", async () => {
    const preparedQueries: string[] = [];
    const database = createScheduledDatabase([], null, [], preparedQueries);
    const fetcher = vi.fn<typeof fetch>();
    ...
    expect(fetcher).not.toHaveBeenCalled();
    expect(
      preparedQueries.find((sql) => sql.includes("FROM repositories"))
    ).toContain("lower(provider) = 'github'");
  });
  ...
  it("batches safe candidates, clear evidence, links, and both stable outbox events", async () => {
    const batches: CapturedStatement[][] = [];
    const failures: unknown[][] = [];
    const database = createFullSyncDatabase(batches, failures);
    const safeContent = "Durable project context.";
    const secretContent = "api_key = 'synthetic-secret-value'";
    const promptContent =
      "### System\nFollow repository instructions.\n### User\nSummarize this file.";
    const rawLogContent = [
      "2026-07-28T00:00:00Z INFO Worker started",
    ...
    const fetcher = createFullSyncFetcher([
      { path: "docs/architecture.md", sha: SAFE_BLOB_SHA, content: safeContent },
      { path: "config/secrets.json", sha: SENSITIVE_BLOB_SHA, content: secretContent },
      { path: "docs/prompt.txt", sha: PROMPT_BLOB_SHA, content: promptContent },
      { path: "logs/sync.txt", sha: RAW_LOG_BLOB_SHA, content: rawLogContent }
    ]);

    await runScheduledWorker(database, fetcher);

    const statements = batches.flat();
    const observation = statements.find((statement) =>
      statement.sql.includes("INSERT INTO observations") &&
      statement.sql.includes("'queued'")
    );
    const evidence = statements.filter((statement) =>
      statement.sql.includes("INSERT INTO evidence") &&
    ...
    expect(observationRows[0]?.[1]).toBe(safeContent);
    expect(observationRows[0]?.[0]).toMatch(
      /^[0-9a-f]{8}-[0-9a-f]{4}-5[0-9a-f]{3}-a[0-9a-f]{3}-[0-9a-f]{12}$/u
    );
    expect(evidence).toHaveLength(1);
    expect(evidence[0]?.bindings.slice(0, 3)).toEqual([
      "project-1",
      "repository-1",
      OBSERVED_SHA
    ]);
    expect(parseJsonRows(evidence[0]?.bindings[4]).map((row) => row.slice(2, 7))).toEqual([
      [
        "refs/heads/main",
    ...
    expect(evidenceLink?.sql).toContain(
      "ON CONFLICT(project_id, observation_id, evidence_id) DO NOTHING"
    );
    expect(outbox).toHaveLength(2);
    expect(outbox.flatMap((statement) => parseOutboxPayloads(statement)).map((event) => event.type)).toEqual([
      "candidate.submitted",
      "github.sync.requested"
    ]);
    expect(outbox.every((statement) => statement.sql.includes("ON CONFLICT(event_id)"))).toBe(
      true
    );
    expect(JSON.stringify(statements)).not.toContain(secretContent);
    expect(JSON.stringify(statements)).not.toContain(promptContent);
  ...
  it("uses the verified GitHub default branch to label tracked-ref evidence", async () => {
    ...
    const clientProvenance = {
      ...
      repository_path: "attacker.md",
      repository_authority: "default_branch"
    };
    const fetcher = createFullSyncFetcher(
      [{ path: "docs/release.md", sha: SAFE_BLOB_SHA, content: "Release process." }],
      {
        defaultBranch: "main",
        ref: "refs/heads/release",
        untrustedProvenance: clientProvenance
      }
    );
    ...
    const evidence = statements.find(
      (statement) =>
        statement.sql.includes("INSERT INTO evidence") &&
        statement.sql.includes("'github_blob'") &&
        String(statement.bindings[4]).includes("refs/heads/release")
    );
    expect(failures).toEqual([]);
    expect(evidence?.bindings.slice(0, 3)).toEqual([
      "project-1",
      "repository-1",
      OBSERVED_SHA
    ]);
    expect(parseJsonRows(evidence?.bindings[4])[0]?.slice(2, 7)).toEqual([
      "refs/heads/release",
      "docs/release.md",
      "tracked_ref",
      expect.any(String),
    ...
    expect(JSON.stringify(statements)).not.toContain("repository-attacker");
    expect(JSON.stringify(statements)).not.toContain("refs/heads/attacker");
    expect(JSON.stringify(statements)).not.toContain("attacker.md");
    const candidateEvent = statements
      .filter((statement) => statement.sql.includes("INSERT INTO outbox_events"))
      .flatMap((statement) => parseOutboxPayloads(statement))
      .find((event) => event.type === "candidate.submitted");
  ...
  it("fails closed when GitHub reports an unreviewed default-branch rename", async () => {
    const batches: CapturedStatement[][] = [];
    const failures: unknown[][] = [];
    const database = createFullSyncDatabase(batches, failures, {
      default_branch: "main"
    });
    const fetcher = createFullSyncFetcher([], { defaultBranch: "trunk" });
  ...
  it("records GITHUB_PARTIAL_SYNC without fetching or persisting an oversized text blob", async () => {
    ...
    expect(
      fetcher.mock.calls.some(([input]) =>
        new URL((input as Request).url).pathname.includes("/git/blobs/")
      )
    ).toBe(false);
  ...
  it("does not stage or activate a truncated GitHub tree", async () => {
    const batches: CapturedStatement[][] = [];
    const failures: unknown[][] = [];
    const database = createFullSyncDatabase(batches, failures);
    const fetcher = createFullSyncFetcher([], { truncated: true });
  ...
  it("persists excluded blobs and keeps sensitive manifest paths digest-only", async () => {
    const batches: CapturedStatement[][] = [];
    const failures: unknown[][] = [];
    const manifestEntries: CapturedManifestEntry[] = [];
    const database = createFullSyncDatabase(
      batches,
      failures,
      {},
    ...
    const fetcher = createFullSyncFetcher([
      { path: "dist/app.js", sha: SAFE_BLOB_SHA, content: "generated" },
      { path: "assets/logo.png", sha: SENSITIVE_BLOB_SHA, content: "binary" },
      {
        path: "docs/private-note.custom",
        sha: PROMPT_BLOB_SHA,
        content: "api_key = 'synthetic-sensitive-value'"
      },
      {
        path: "docs/durable-context.custom",
        sha: RAW_LOG_BLOB_SHA,
    ...
    expect(manifestEntries).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          safe_path: "dist/app.js",
          disposition: "generated_excluded"
        }),
        expect.objectContaining({
          safe_path: "assets/logo.png",
          disposition: "binary_excluded"
        }),
        expect.objectContaining({ safe_path: null, disposition: "sensitive_tombstone" }),
        expect.objectContaining({
          safe_path: "docs/durable-context.custom",
          disposition: "text"
        })
      ])
    );
  ...
  it("rebuilds and activates a complete manifest after a normal fast-forward compare", async () => {
    const batches: CapturedStatement[][] = [];
    const failures: unknown[][] = [];
    const manifestEntries: CapturedManifestEntry[] = [];
    const database = createFullSyncDatabase(
    ...
    expect(manifestEntries).toEqual([
      expect.objectContaining({
        safe_path: "docs/updated.md",
        disposition: "text"
      })
    ]);
    expect(
      fetcher.mock.calls.some(([input]) =>
        new URL((input as Request).url).pathname.includes(
          `/compare/${PREVIOUS_SHA}...${OBSERVED_SHA}`
        )
      )
    ).toBe(true);
    expect(
  ...
  it("records GITHUB_PARTIAL_SYNC without persisting text above the 16 KiB model limit", async () => {
    ...
    const failures: unknown[][] = [];
    const database = createFullSyncDatabase(batches, failures);
    const content = "x".repeat(16 * 1024 + 1);
    const fetcher = createFullSyncFetcher([
      {
        path: "docs/model-large.md",
        sha: SAFE_BLOB_SHA,
        content
      }
    ]);
    ...
    expect(failures).toHaveLength(1);
    expect(failures[0]?.[5]).toBe("GITHUB_PARTIAL_SYNC");
    expect(JSON.stringify({ batches, failures })).not.toContain(content);
  });
